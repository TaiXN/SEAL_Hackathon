...
export function AuthLayout() {
  ...
  const handleLoginSubmit = async (e: React.FormEvent) => {
    ...
    try {
      let data; // hứng data từ axios
      let navigateTo = "";

      // 1. Phân luồng gọi API siêu gọn gàng bằng authApi
      if (role === "admin") {
        data = await authApi.loginAdmin(credentials);
        navigateTo = "/admin/dashboard"; // Sửa lại đúng link xịn của Admin
      } else if (role === "judge") {
        data = await authApi.loginTeacher(credentials);
        navigateTo = "/judge";
      } else {
        // data = await authApi.loginMember(credentials);
        // navigateTo = "/gateway"; // Sửa lại link cho member
        throw new Error("Chức năng đăng nhập cho Member chưa hoàn thiện!");
      }

      // Bắt buộc phải check xem data có tồn tại không để TypeScript hết gạch đỏ
      if (!data) {
        throw new Error("Không nhận được dữ liệu từ Server");
      }
      console.log("Data API trả về nè: ", data);

      // 2. lấy token: axios ép thành json r
      const actualToken = data.accessToken;
      setTokens(actualToken, role);

      // Tắt thông báo "Đang tải" và bật thông báo Thành Công xanh lá cây!
      toast.success("Đăng nhập thành công! Đang chuyển hướng...", {
        id: loadingToastId, // Gắn ID để nó ghi đè lên cái toast loading
      });

      // alert("Đăng nhập thành công!"); // (Thực tế làm Dân Pro người ta xài Toast Notification chữ mờ mờ góc màn hình chứ ai xài alert nữa bà 😂)
      navigate(navigateTo);
